...
def test_get_block_storage_dispatches_to_filesystem_by_default(tmp_path):
    settings = Settings()
    settings.path.blocks_dir = str(tmp_path / "blocks")
...
def test_get_result_storage_dispatches_to_filesystem_by_default(tmp_path):
    settings = Settings()
    settings.path.results_dir = str(tmp_path / "results")
...
def test_get_block_storage_dispatches_to_sqlite(tmp_path):
    settings = Settings()
    settings.path.storage.backend = "sqlite"
    settings.path.storage.sqlite_path = str(tmp_path / "cvdocs.db")
...
def test_get_block_storage_raises_a_clear_error_for_an_unknown_backend():
    settings = Settings()
    settings.path.storage.backend = "postgres"
...
def test_get_auth_provider_dispatches_to_installed_app_for_filesystem(tmp_path):
    settings = Settings()
    settings.auth.google.token_path = str(tmp_path / "token.json")
