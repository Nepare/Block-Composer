...
def validate_block_shape(body: str) -> None:
    """Shared by generate and mutate: body must have a heading and at least one labeled
    field to count as a plausibly real entry."""
    stripped = body.strip()
    if not stripped:
...
def run_generate(
    ...
    *,
    ...
    try:
        ...
        body = client.chat(retry_messages, model, temperature=0.5, max_tokens=settings.behavior.generate.max_tokens, reasoning_effort="low")
        validate_block_shape(body)
