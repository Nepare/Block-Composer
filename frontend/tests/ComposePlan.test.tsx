...
test("a finalize step renders with its own label and the same status treatment as every other step", () => {
  const planSteps = [
    step({ order: 1, liveStatus: "done", action: "use", blockId: "b1" }),
    step({ order: 2, liveStatus: "running", action: "finalize" }),
  ];
  ...
  expect(rows[1]).toHaveAttribute("data-status", "running");
  expect(rows[1]).toHaveTextContent("Finalizing");
  expect(rows[1].querySelector("svg")).not.toBeNull();
});
